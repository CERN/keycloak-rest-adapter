...
def get_admin_access_token(keycloak_server, realm, admin_user, admin_password):
    ...
    url = "https://{0}/auth/realms/{1}/protocol/openid-connect/token".format(keycloak_server, realm)
...
def get_admin_access_token_headers(keycloak_server, realm, admin_user, admin_password):
    ...
    """
    access_token_object = get_admin_access_token(keycloak_server, realm, admin_user, admin_password)
...
class TestKeycloakClient(unittest.TestCase):
    def setUp(self):
        ...
        self.headers = get_admin_access_token_headers(keycloak_server, realm, admin_user, admin_password)
        send_request('delete', "{0}/client/{1}".format(self.baseurl, self.clientId), headers=self.headers).text
    ...
    def test_03_delete_client_return_not_found(self):
        ...
        self.assertEqual(response, "Cannot delete client '{0}'. Client not found".format(non_exisiting_client))
    ...
    def test_04_create_saml_client_with_protocol_return_created_client(self):
        ...
        self.assertEqual(json.loads(new_client)["clientId"], self.clientId)
        # annnd delete client
        send_request('delete', "{0}/client/{1}/{2}".format(self.baseurl, protocol, self.clientId)).text
    ...
    def test_05_create_openid_client_with_protocol_return_created_client(self):
        ...
        self.assertEqual(json.loads(new_client)["clientId"], self.clientId)
        # annnd delete client
        send_request("delete", "{0}/client/{1}/{2}".format(self.baseurl, protocol, self.clientId), headers=self.headers).text
    ...
    def test_07_regerentate_client_secret_changes_secret(self):
        ...
        new_secret = send_request("post", "{0}/client/openid/{1}/regenerate-secret".format(self.baseurl, self.clientId), headers=self.headers).text
